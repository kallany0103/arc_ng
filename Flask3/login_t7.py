...
def login():
    ...
    if user and check_password_hash(user.password, password):
        access_token = create_access_token(identity=user.user_id, additional_claims={'role': user.role.role_name})
...
@jwt_required()
@role_required('admin', 'user')  # Example roles that can access this route
def create_author():
    try:
        current_user_id = get_jwt_identity()
        data = request.get_json()
