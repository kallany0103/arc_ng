...
@app.route('/get_arcusers', methods=['GET'])
def get_arcusers():
    try:
        # Make a GET request to the Express.js API endpoint
        response = requests.get('http://127.0.0.1:9000/arcusers')
...
def add_user():
    try:
        ...
        # Make a POST request to the requested API endpoint
        response = requests.post('http://127.0.0.1:9000/addUser', json=data)
        return response.json()
...
def login_user():
    try:
        data = request.get_json()
        response = requests.post('http://127.0.0.1:9000/login',  json=data)
